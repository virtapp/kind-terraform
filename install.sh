...
helm install csi-secrets-store secrets-store-csi-driver/secrets-store-csi-driver && sleep 5
         echo    Waiting for all pods in running mode:
until kubectl wait --for=condition=Ready pods --all -n vault; do
sleep 2
done  2>/dev/null
...
sleep 5 && kubectl create namespace monitor || true
...
terraform state list && terraform providers
